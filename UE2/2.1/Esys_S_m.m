...
function Output(block)
  ...
  % TODO: Port 2:
  C1 = C1_ref - kC1 * UC1_ref + kC1 * UC1; % Aktueller Wert der differentiellen Kapazitaet C1
  y2(1) = C1;
  
  % TODO: Port 3:
  Ua = UC2 * K; % Ausgangsspannung
  y3(1) = Ua; 
...
function Derivatives(block)
  ...
  dQ1 = C1_ref - kC1 * UC1_ref + kC1 * UC1;
  
  % TODO: Berechnen der Zeitableitungen der Zustaende
  dx(1) = 1/dQ1 * (Ue/R1 - UC1*(R1+R2)/(R1*R2) - UC2*(K*(R1+R2)-R1)/(R1*R2));
  dx(2) = UC1/(C2*R2) + UC2*(K-2)/(C2*R2) + Us/(C2*R2);
